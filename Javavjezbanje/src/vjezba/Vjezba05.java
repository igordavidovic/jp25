...
public class Vjezba05 {
	...
	int broj2 = Integer.parseInt(JOptionPane.showInputDialog("Upiši koliko će brojeva biti u nizu"));
		...
		boolean prv1 = false;
		for(int i = 0;i < broj7;i++) {
			for(int j = 0;j < broj8;j++) {
				niz7[i][j] = Integer.parseInt(JOptionPane.showInputDialog("Upiši broj"));
			}
		}
		int tb = Integer.parseInt(JOptionPane.showInputDialog("Upiši broji koji će se tražiti u matrici"));
		
		for(int i = 0;i < broj7;i++) {
			System.out.println(Arrays.toString(niz7[i]));
		}
		vanjska2:
		for(int i = 0;i < broj7;i++) {
			for(int j = 0;j < broj8;j++) {
